...
-- ============================================================================
-- Section 2b: bulk job outbox for reliable enqueueing
-- ============================================================================
...
CREATE INDEX bulk_job_outbox_pending_idx ON bulk_job_outbox (status) WHERE status = 'PENDING';

CREATE TRIGGER bulk_job_outbox_updated_at_trg
    BEFORE UPDATE ON bulk_job_outbox
    FOR EACH ROW
    EXECUTE FUNCTION set_updated_at();

-- ============================================================================
-- Section 2c: dead letter for permanently failed items
-- ============================================================================

CREATE TABLE bulk_job_dead_letter (
    id              UUID        NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    item_id         UUID        NOT NULL REFERENCES bulk_job_items(id) ON DELETE SET NULL,
    job_id          UUID        NOT NULL REFERENCES bulk_jobs(id) ON DELETE CASCADE,
    payload         JSONB       NOT NULL,
...
CREATE INDEX bulk_job_dead_letter_job_idx ON bulk_job_dead_letter (job_id);
