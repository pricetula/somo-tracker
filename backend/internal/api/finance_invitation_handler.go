...
type FinanceInvitationHandler struct {
...
func NewFinanceInvitationHandler(svc services.FinanceInvitationServiceInterface, cli *stytch.Client, asynqClient *asynq.Client, redisClient *redis.Client, logger *zap.Logger) *FinanceInvitationHandler {
...
func (h *FinanceInvitationHandler) HandleInvites(c fiber.Ctx) error {
	...
	if h.asynq != nil {
		...
		for b := 0; b < totalBatches; b++ {
			...
			_, enqueueErr := h.asynq.EnqueueContext(c.Context(), asynq.NewTask("finance:invitation:batch", payload), asynq.Queue("finance_invitation"), asynq.TaskID(fmt.Sprintf("%s_%d", jobID.String(), b)))
			if enqueueErr != nil {
