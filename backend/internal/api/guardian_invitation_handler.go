...
type GuardianInvitationHandler struct {
	svc    services.GuardianInvitationServiceInterface
...
func NewGuardianInvitationHandler(svc services.GuardianInvitationServiceInterface, cli *stytch.Client, asynqClient *asynq.Client, redisClient *redis.Client, logger *zap.Logger) *GuardianInvitationHandler {
	return &GuardianInvitationHandler{svc: svc, stytch: cli, asynq: asynqClient, redis: redisClient, logger: logger.With(zap.String("handler", "guardian_invitation"))}
}

// @Summary Bulk invite guardians
// @Tags Guardian Invitations
// @Accept json
// @Produce json
...
// @Router /api/guardians/invitations [post]
func (h *GuardianInvitationHandler) HandleInvites(c fiber.Ctx) error {
	// Extract locals
	...
	if h.asynq != nil {
		...
		for b := 0; b < totalBatches; b++ {
			...
			_, enqueueErr := h.asynq.EnqueueContext(c.Context(), asynq.NewTask("guardian:invitation:batch", payload), asynq.Queue("guardian_invitation"), asynq.TaskID(fmt.Sprintf("%s_%d", jobID.String(), b)))
			if enqueueErr != nil {
...
// @Router /api/guardians/invitations/jobs/{job_id} [get]
func (h *GuardianInvitationHandler) GetJob(c fiber.Ctx) error {
...
// @Router /api/guardians/invitations/jobs/{job_id}/retry-failed [post]
func (h *GuardianInvitationHandler) RetryFailed(c fiber.Ctx) error {
	jobIDStr := c.Params("job_id")
	jobID, err := uuid.Parse(jobIDStr)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"code": "bad_request", "message": "invalid job_id", "errors": fiber.Map{}})
	...
	if h.asynq != nil {
		...
		_, enqueueErr := h.asynq.EnqueueContext(c.Context(), asynq.NewTask("guardian:invitation:retry", payload), asynq.Queue("guardian_invitation"))
		if enqueueErr != nil {
...
// @Router /api/guardians/invitations/jobs/{job_id}/events [get]
func (h *GuardianInvitationHandler) Events(c fiber.Ctx) error {
