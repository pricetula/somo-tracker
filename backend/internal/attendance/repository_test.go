...
func TestPgRepository_GetMarkedTimetableAllocation_RosterAndRecords(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
	ctx := context.Background()
	...
	academicYearID := uuid.New().String()
	_, err := pool.Exec(ctx, `INSERT INTO academic_years (id, tenant_id, school_id, name, start_date, end_date, is_current, created_by, updated_by) VALUES ($1,$2,$3,'2026','2026-01-01','2026-12-31',true,$4,$4)`, academicYearID, tenantID, schoolID, userID)
	require.NoError(t, err)

	academicTermID := uuid.New().String()
	_, err = pool.Exec(ctx, `INSERT INTO academic_terms (id, tenant_id, school_id, academic_year_id, name, term_number, start_date, end_date, is_current, is_final, created_by, updated_by) VALUES ($1,$2,$3,$4,'T1',1,'2026-01-01','2026-03-31',true,false,$5,$5)`, academicTermID, tenantID, schoolID, academicYearID, userID)
	require.NoError(t, err)
	...
	_, err = pool.Exec(ctx, `INSERT INTO cbc_students (id, tenant_id, school_id, full_name, gender, admission_number, is_active) VALUES ($1,$2,$3,'Amina','F','ADM001',true)`, studentID, tenantID, schoolID)
	require.NoError(t, err)

	_, err = pool.Exec(ctx, `INSERT INTO cbc_student_enrollments (id, tenant_id, student_id, school_id, academic_term_id, academic_year_id, class_id, status) VALUES ($1,$2,$3,$4,$5,$6,$7,'ACTIVE')`, uuid.New().String(), tenantID, studentID, schoolID, academicTermID, academicYearID, classID)
	require.NoError(t, err)
	...
	// Case 1: no session, no attendance record -> roster returns student, empty status
	resp, err := repo.GetMarkedTimetableAllocation(ctx, tenantID, schoolID, allocationID, academicTermID, "2026-02-10")
	require.NoError(t, err)
	require.Len(t, resp.Students, 1)
	require.Equal(t, studentID, resp.Students[0].StudentID)
	require.Equal(t, "", resp.Students[0].Status)
	require.Nil(t, resp.SessionID)

	// Case 2: create session SUBMITTED + attendance record PRESENT with note
	...
	require.NoError(t, err)
	require.NotNil(t, resp2.SessionID)
	require.Equal(t, "SUBMITTED", *resp2.SessionStatus)
	require.Len(t, resp2.Students, 1)
	require.Equal(t, "PRESENT", resp2.Students[0].Status)
	require.NotNil(t, resp2.Students[0].Note)
	require.Equal(t, "late", *resp2.Students[0].Note)
}
