...
type TeacherInvitationServiceInterface interface {
	...
	UserHasTeacherRole(ctx context.Context, schoolID, userID uuid.UUID) (bool, error)
...
func NewTeacherInvitationService(pool *pgxpool.Pool, logger *zap.Logger) TeacherInvitationServiceInterface {
	return &TeacherInvitationService{
		GenericInvitationService: NewGenericInvitationService(pool, logger, "TEACHER_INVITATION", "TEACHER"),
	}
}

func (s *TeacherInvitationService) UserHasTeacherRole(ctx context.Context, schoolID, userID uuid.UUID) (bool, error) {
	...
	// For invitation creation, only admins allowed (or could allow teachers managing teachers)
	return false, nil
}
