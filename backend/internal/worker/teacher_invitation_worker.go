...
type TeacherInvitationProcessor struct {
...
func NewTeacherInvitationProcessor(svc services.TeacherInvitationServiceInterface, cli *stytch.Client, logger *zap.Logger, pub RedisPublisher) *TeacherInvitationProcessor {
	return &TeacherInvitationProcessor{
		svc:       svc,
		stytchCli: cli,
...
// ProcessTask handles "teacher:invitation:batch" tasks.
// Processes each item sequentially, updating status and publishing progress.
func (p *TeacherInvitationProcessor) ProcessTask(ctx context.Context, task *asynq.Task) error {
...
func (p *TeacherInvitationProcessor) ProcessRetryTask(ctx context.Context, task *asynq.Task) error {
...
func (p *TeacherInvitationProcessor) processSingleItem(ctx context.Context, jobID, itemID uuid.UUID) {
...
func (p *TeacherInvitationProcessor) handleStytchError(ctx context.Context, jobID, itemID uuid.UUID, attempts int, err error, payload InvitationPayload) {
...
func (p *TeacherInvitationProcessor) deriveJobStatus(ctx context.Context, jobID uuid.UUID) {
...
func (p *TeacherInvitationProcessor) publishProgress(ctx context.Context, jobID uuid.UUID) {
	if p.redis == nil {
		return
	}
