...
- `trg_timetable_blocks_updated_at` BEFORE UPDATE ON timetable_blocks FOR EACH ROW EXECUTE FUNCTION fn_set_updated_at()

### CBC Timetable Allocations
...
- `trg_timetable_allocations_updated_at` BEFORE UPDATE ON timetable_allocations FOR EACH ROW EXECUTE FUNCTION fn_set_updated_at()
...
- `uq_cbc_attendance_sessions_slot_date` UNIQUE (school_id, timetable_allocation_id, date)
- `fk_cbc_attendance_sessions_tenant_slot` FOREIGN KEY (tenant_id, timetable_allocation_id) REFERENCES timetable_allocations(tenant_id, id) ON DELETE CASCADE
...
- `idx_cbc_attendance_sessions_slot_date` (timetable_allocation_id, date)
...
- `uq_attendance_student_slot_date` UNIQUE (student_id, timetable_allocation_id, date)
- `fk_attendance_tenant_student` FOREIGN KEY (tenant_id, student_id) REFERENCES cbc_students(tenant_id, id) ON DELETE CASCADE
- `fk_attendance_timetable_allocation` FOREIGN KEY (tenant_id, timetable_allocation_id) REFERENCES timetable_allocations(tenant_id, id) ON DELETE CASCADE
...
- TABLE attendance_records: 'Per-student, per-timetable-slot, per-date attendance records. The unique constraint (student_id, timetable_allocation_id, date) prevents duplicate marks. Only created for slots where timetable_blocks.is_break = false.'
...
- `idx_attendance_slot_date` (timetable_allocation_id, date)
...
- `fk_behavior_notes_timetable_allocation` FOREIGN KEY (tenant_id, timetable_allocation_id) REFERENCES timetable_allocations(tenant_id, id) ON DELETE CASCADE
...
- `idx_behavior_notes_slot_date` (timetable_allocation_id, date)
...
- Users → Class Teachers, Timetable Allocations (teacher_id)
...
- Classes ↔ Learning Areas ↔ Teachers (via timetable_allocations)
...
- Timetable Blocks (tenant_id, id)
- CBC Timetable Allocations (tenant_id, id)
...
- **CASCADE**:
    ...
    - Classes → Class Teachers, Timetable Allocations, Attendance Sessions, Enrollments (class_id SET NULL)
    ...
    - Timetable Blocks → Timetable Allocations
...
- Attendance Sessions (school_id, timetable_allocation_id, date)
...
- Behavior Notes → Students, Timetable Allocations, Behavior Categories, Users (authored_by/reviewed_by)
...
- `timetable_blocks`: end_time > start_time; day_of_week between 1 and 7
