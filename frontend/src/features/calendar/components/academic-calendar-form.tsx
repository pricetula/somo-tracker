...
import { CheckCircle2, Flag, Loader2, Plus, Trash2 } from "lucide-react";
...
const formSchema = z
    ...
    .refine(
        (data) => {
            ...
            return data.periods.every((p) => p.startDate && p.endDate && p.endDate > p.startDate);
        },
...
export function AcademicCalendarForm({ onSuccess }: AcademicCalendarFormProps) {
    ...
    const watchedPeriods = useWatch({ control: form.control, name: "periods" });
    ...
    return (
        <div
            className={`bg-card rounded-2xl border p-6 shadow-sm transition-all duration-300 ${
                        ...
                        <div className="space-y-4">
                            {fields.map((field, index) => {
                                ...
                                if (index > 0 && watchedPeriods[index - 1]?.endDate) {
                                    ...
                                    minStart =
                                        nextDay > (yearStart ?? nextDay) ? nextDay : yearStart;
                                }
                                ...
                                return (
                                    <div
                                        key={field.id}
                                        className="bg-muted/30 flex flex-wrap items-end gap-3 rounded-lg border p-4"
                                    >
                                        {/* Period Name */}
                                        <FormField
                                            control={form.control}
                                                    ...
                                                    <FormControl>
                                                        <Button
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                form.trigger(
                                                                    `periods.${index}.isFinal`
                                                                );
                                                            }}
                                                        >
                                                            <Flag
                                                                className={`h-3.5 w-3.5 ${field.value ? "fill-current" : ""}`}
                                                            />
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                ...
                                                const currentFinal =
                                                    form.getValues("periods")[index]?.isFinal;
                                                remove(index);
                                                if (currentFinal) {
                                                    ...
                                                    setTimeout(() => {
                                                        ...
                                                        if (updated.length > 0) {
                                                            form.setValue(
                                                                `periods.${updated.length - 1}.isFinal`,
                                                                true
                                                            );
                                                        }
                                                    }, 0);
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                const demoPeriods = [
                                    {
                                        start: new Date(demoStart),
                                        end: new Date(selectedYear, 3, 11),
                                    }, // Jan 6 – Apr 11
                                    {
                                        start: new Date(selectedYear, 4, 5),
                                        end: new Date(selectedYear, 7, 15),
                                    }, // May 5 – Aug 15
                                    {
                                        start: new Date(selectedYear, 8, 1),
                                        end: new Date(selectedYear, 10, 28),
                                    }, // Sep 1 – Nov 28
                                ];
