...
import { fetchCurrentCalendar, saveAcademicCalendar } from "@/lib/api/academic-calendar";
...
export function useSaveAcademicCalendar() {
    ...
    return useMutation({
        mutationFn: (payload: CreateAcademicCalendarPayload) => saveAcademicCalendar(payload),
        onSuccess: async () => {
