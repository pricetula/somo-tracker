...
export function ClassStreamGenerator({ onSuccess }: ClassStreamGeneratorProps) {
    ...
    return (
        <div
            ref={containerRef}
            className={`bg-card rounded-2xl border p-6 shadow-sm transition-all duration-500 ${
                isSubmitting ? "pointer-events-none opacity-60" : ""
            } ${fadeOut ? "scale-95 opacity-0" : ""}`}
        >
            {/* Header */}
            ...
            <div className="mb-4">
                ...
                <div className="border-input bg-background focus-within:ring-ring flex flex-wrap items-center gap-2 rounded-lg border px-3 py-2 focus-within:ring-2 focus-within:ring-offset-1">
                    {streams.map((stream, index) => (
                        <span
                            key={stream}
                            className="bg-primary/10 text-primary inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-sm font-medium"
                        >
                            {stream}
                            <button
                                type="button"
                                onClick={() => removeStreamTag(index)}
                                className="text-primary/60 hover:text-primary inline-flex items-center rounded-sm p-0.5 focus:outline-none"
                    ...
                    <input
                        ...
                        disabled={isSubmitting}
                        className="placeholder:text-muted-foreground min-w-[200px] flex-1 border-0 bg-transparent p-0 text-sm outline-none"
                    />
                </div>
                <p className="text-muted-foreground mt-1.5 text-xs">
                    Press Enter to add a stream. You can also paste multiple names
                    (comma-separated).
                </p>
            </div>
            ...
            {previewGrid.length > 0 && (
                <div className="mb-6">
                    ...
                    <div className="bg-muted/30 grid grid-cols-2 gap-2 rounded-xl border p-4 sm:grid-cols-3 md:grid-cols-4">
                        {previewGrid.map((name) => (
                            <div
                                key={name}
                                data-testid="preview-cell"
                                className="bg-background text-foreground hover:border-primary/30 rounded-lg border px-3 py-2 text-center text-sm font-medium shadow-sm transition-colors"
                            >
                                {name}
                            </div>
                        ))}
            ...
            {previewGrid.length === 0 && (
                <div className="bg-muted/20 mb-6 flex items-center justify-center rounded-xl border border-dashed p-8">
            ...
            <div className="flex items-center justify-between">
                <Button onClick={handleSubmit} disabled={!canSubmit} className="min-w-[220px]">
                    {isSubmitting ? (
                        <>
