...
import { useBulkInviteGuardians } from "../hooks/use-guardian-invitations";
import type { MappedRow } from "@/features/import/components/upload/field-mapper";

const GUARDIANINVITE_FIELDS = [
    {
        key: "full_name",
        label: "Full name",
...
export function GuardianInviteOrchestrator() {
    const mutation = useBulkInviteGuardians();
    ...
    const handleMappedList = async (rows: MappedRow[]) => {
        ...
        const key = `bulk-guardian-invite:${hash}`;

        try {
            sessionStorage.setItem(`bulk-guardian-invite-key:${hash}`, key);
        } catch {}
    ...
    return (
        <ImportOrchestrator
            fieldDef={GUARDIANINVITE_FIELDS}
            onMappedList={handleMappedList}
            isSubmitting={mutation.isPending}
            progressUrl={(jobId) => `/backend/api/guardians/invitations/jobs/${jobId}/events`}
            showProgress
            onReset={handleReset}
        />
    );
